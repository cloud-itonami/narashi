...
(def ^:private root (.. here getParentFile getParentFile getParentFile)) ;; methods -> narashi -> test -> repo
(def ^:private lexdir (java.io.File. root "lex"))

(defn- read-edn [file] (edn/read-string (slurp file)))
(defn- manifest [] (read-edn (java.io.File. root "manifest.edn")))
(defn- lex [name] (read-edn (java.io.File. lexdir (str name ".edn"))))
...
(deftest test-all-4-lexicons-resolve
  ...
  (doseq [nsid (get (manifest) "lexiconNamespaces")]
    (let [short (last (str/split nsid #"\."))]
      ...
      (is (= nsid (get (lex short) "id")) (str short ".edn id must equal its declared namespace")))))
